...
function proxyImageRequest(imageUrl, resp) {
  ...
  if (!url || !url.protocol) {
    writeResponse(resp, 400, `invalid param: ${imageUrl}`);
    return;
  }
  ...
  let req = client.get(url, (upstreamResp) => {
    ...
    // - Content-Type check
    // - size limit
    if (upstreamResp.statusCode == 301 || upstreamResp.statusCode == 302) {
      writeResponse(resp, 502, `unsupported upstream response (HTTP ${upstreamResp.statusCode})`);
      return;
    }
